...
class RtreeFile:
    ...
    def __init__(self, filename="RtreeFile.bin", page_manager=None, io_counter=None):
        os.makedirs(FILE_DIR, exist_ok=True)
        self.FILE_PATH = os.path.join(FILE_DIR, filename)
        
        # Inicializar PageManager con tamaño de página 8192 para R-Tree
        self.pm = page_manager or PageManager(self.FILE_PATH, io_counter=io_counter, page_size=8192)
        self.io_counter = io_counter
        
        # Contadores independientes de I/O para este índice
        self.read_count = 0
        self.write_count = 0
        ...
        try:
            ...
            self.FILE_HEADER = FileHeader(page_size=8192)
    ...
    def read_page(self, page_id: int) -> Page:
        # Mapeo: RtreeFile page_id -> PageManager page_id
        # page_id 0 (RtreeFile) -> página 1 (PageManager)
        # page_id n (RtreeFile) -> página (n+1) (PageManager)
        pm_page_id = page_id + 1
        
        data = self.pm.read_page(pm_page_id)
        
        # Rastrear I/O independiente
        self.read_count += 1
    ...
    def write_page(self, page: Page):
        ...
        pm_page_id = page.PAGE_ID + 1
        self.pm.write_page(pm_page_id, bytes(page.DATA))
    ...
    def read_file_header(self) -> None:
        # Lee el header de la página 0
        page0 = self.pm.read_page(0)
        header_data = page0[:FILE_HEADER_SIZE]
        
        if header_data == b'\x00' * FILE_HEADER_SIZE:
            # Header vacío, será inicializado
            raise FileNotFoundError("Header no inicializado")
    ...
    def write_file_header(self) -> None:
        # Lee página 0, actualiza header y escribe
        ...
        page0[:FILE_HEADER_SIZE] = header_bytes
        self.pm.write_page(0, bytes(page0))
    
    def get_io_stats(self) -> dict:
        """Retorna estadísticas de I/O independientes del R-Tree"""
        return {
    ...
    def reset_counters(self) -> None:
        """Reinicia los contadores de I/O independientes"""
        self.read_count = 0
        self.write_count = 0
...
class Rtree:
    ...
    def __init__(self, filename="RtreeFile.bin", page_manager=None, io_counter=None):
        self.RTREE_FILE = RtreeFile(filename, page_manager=page_manager, io_counter=io_counter)
        self.io_counter = io_counter
    ...
    def get_io_stats(self) -> dict:
        return self.RTREE_FILE.get_io_stats()
    
    def get_global_io_stats(self) -> dict:
        if self.io_counter:
            return self.io_counter.snapshot()
        return None
    
    def reset_io_counters(self) -> None:
        self.RTREE_FILE.reset_counters()